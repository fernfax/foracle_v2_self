...
export function PoliciesClient({
  ...
  const primaryCoverageOf = (policy: Policy): number => {
    ...
    try {
      const options = JSON.parse(policy.coverageOptions) as Record<
        string,
        unknown
      >
    ...
    <div className="space-y-4">
            ...
            {Object.entries(policyTypeCounts)
              ...
              .map(([policyType, count]) => (
                <Button
                  key={policyType}
                  variant={
                    selectedPolicyType === policyType ? "default" : "outline"
                  }
                  size="sm"
      ...
      {policies.length === 0 && familyMembers.length === 0 ? (
        ...
        <div className="space-y-8">
          {sortedPoliciesByMember.map(
            ({ member, policies: memberPolicies }) => {
                    ...
                    <div>
                      <h2 className="text-foreground text-xl font-semibold">
                        ...
                        {member.relationship?.toLowerCase() === "self" && (
                          <span className="text-foreground/400 ml-2 text-sm font-normal">
                            (You)
                          </span>
                        )}
                      </h2>
                      <p className="text-foreground/400 text-sm">
                        ...
                        {memberPolicies.length === 1 ? "policy" : "policies"}
                      </p>
                    </div>
                  </div>
      ...
      <ConfirmDialog
        ...
        description={
          <>
            &ldquo;
            {deletingPolicy?.planName?.trim() || deletingPolicy?.policyType}
            &rdquo; will be removed. This can&rsquo;t be undone.
          </>
        }
