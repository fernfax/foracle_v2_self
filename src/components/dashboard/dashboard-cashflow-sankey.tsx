...
import { CHART_PALETTE, STATUS_COLORS } from "@/lib/charts/chart-palette"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
function colorForKind(kind: ModelNode["kind"]): string {
  switch (kind) {
    case "cpf":
      return "#B8622A" // brand-terracotta
    case "savings":
      return STATUS_COLORS.positive
    case "shortfall":
      return STATUS_COLORS.danger
    case "income":
      return "#3A6B52" // brand-jungle
    case "hub":
      return "#2C3E3D" // forest-mid
    case "category":
    case "item":
...
export function CashflowSankey({
  ...
  const [confirmNav, setConfirmNav] = useState<"incomes" | "expenses" | null>(
    null
  )
  ...
  const realValueById = useMemo(() => {
    ...
    for (const c of model.categories)
      for (const i of c.items) m.set(i.id, i.value)
    return m
  }, [model])
  ...
  const expandedCategory = expandedId
    ? (model.categories.find((c) => c.id === expandedId) ?? null)
    : null
  ...
  const showNetTotal =
    model.totalCpf > 0 && totalNetIncome < model.totalGross - 0.5
  ...
  const data = useMemo(() => {
    ...
    for (const link of model.links) {
      ...
      const linkValue = visualize(link.value) * (isFromHub ? outflowScale : 1)
      links.push({
        source: s,
        target: t,
        value: linkValue,
            ...
            (showNet ? (
              ...
              <text
                ...
                <tspan
                  ...
                  {fmt(realValue)} · {fmt(netValue ?? realValue)}
                  {isNarrow ? "" : " take-home"}
                </tspan>
              </text>
            ) : isNarrow ? (
              ...
              <text
                ...
                style={transition}
                opacity={isDimTarget ? 0.25 : 1}>
                <tspan x={labelX} dy={-2} fontWeight={600}>
                  {truncateLabel(n.name, 12)}
                </tspan>
                <tspan x={labelX} dy={11} fill="hsl(var(--muted-foreground))">
                  {fmt(realValue)}
                </tspan>
              </text>
            ) : (
              <text
                ...
                <tspan dx={6} fill="hsl(var(--muted-foreground))">
                  {fmt(realValue)}
                </tspan>
              </text>
            ))}
    ...
    if (isFocusRibbon) {
      ...
      if (cat && cat.value > 0 && cat.items.length > 0) {
        ...
        const contentW = Math.max(
          1,
          linkWidth - GAP * Math.max(0, itemCount - 1)
        )
    ...
    <Card>
          ...
          <div className="min-w-0 flex-1">
            ...
            {isProjection ? (
              <p className="text-muted-foreground mt-1 text-xs">
                Projected balance based on current recurring income, expenses,
                and one-off items.
              </p>
            ) : expandedCategory ? (
              <p className="text-muted-foreground mt-1 text-xs">
              ...
              <p className="text-muted-foreground mt-1 text-xs">
                Gross income flows into the pool, then out to CPF, your expense
                categories, and savings. Click a category to break it down.
                Hover any node or ribbon for detail.
              </p>
            )}
          </div>

          <div
            data-tour="cashflow-controls"
      ...
      <CardContent>
        {isProjection ? (
          ...
          <div className="text-muted-foreground flex h-[320px] items-center justify-center px-6 text-center text-sm">
            No income recorded for {formatMonthDisplay(selectedMonth)} — try a
            different month, switch to Projection, or add an income.
          </div>
        ) : (
          <>
            <div
              ...
              className="relative h-[340px] w-full sm:h-[calc(90vh-198px)] sm:min-h-[400px]"
            ...
            <div className="font-display mt-1 flex items-center justify-between gap-3 pr-12 text-xs sm:pr-0 sm:text-sm">
            ...
            <div className="text-muted-foreground mt-3 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-xs">
              ...
              {model.shortfall > 0 && (
                <LegendDot color={STATUS_COLORS.danger} label="Shortfall" />
              )}
            </div>
          </>
        )}
      ...
      {itemHover &&
        ...
        createPortal(
          <div
            role="tooltip"
            className="bg-card text-card-foreground border-border animate-in fade-in pointer-events-none rounded-lg border p-3 text-sm shadow-lg"
            ...
            <div className="text-muted-foreground space-y-0.5 text-xs">
              ...
              <div>
                ...
                <span className="text-foreground font-medium tabular-nums">
                  {(() => {
                    const cat = model.categories.find(
                      (c) => c.name === itemHover.parentName
                    )
                    const denom = cat?.value ?? 0
                    return denom > 0
                      ? `${((itemHover.itemValue / denom) * 100).toFixed(1)}%`
                      : "—"
                  })()}
                </span>
              </div>
              <div className="border-border/50 mt-1 border-t pt-1 text-[11px]">
                Click to collapse
              </div>
            </div>
          </div>,
      ...
      {nodeTip &&
        ...
        createPortal(
          <div
            role="tooltip"
            className="bg-card text-card-foreground border-border animate-in fade-in rounded-lg border p-3 text-sm shadow-lg"
            style={{
              ...tooltipStyle(nodeTip.x, nodeTip.y),
              pointerEvents: "auto"
            }}
            ...
            <div className="text-muted-foreground space-y-0.5 text-xs">
              ...
              {nodeTip.kind !== "shortfall" && nodeTip.kind !== "hub" && (
                <div>
                  ...
                  <span className="text-foreground font-medium tabular-nums">
                    {(realTotalGross > 0
                      ? (nodeTip.realValue / realTotalGross) * 100
                      : 0
                    ).toFixed(1)}
                    %
                  </span>
              ...
              {nodeTip.kind === "income" &&
                nodeTip.meta?.cpf != null &&
                nodeTip.meta.cpf > 0 && (
                  <div className="border-border/50 mt-1 border-t pt-1">
                    <div>
                      Gross:{" "}
                      <span className="text-foreground tabular-nums">
                        {fmt(nodeTip.meta.gross ?? 0)}
                      </span>
                    </div>
                    <div>
                      Employee CPF:{" "}
                      <span className="text-foreground tabular-nums">
                        {fmt(nodeTip.meta.cpf)}
                      </span>
                    </div>
                    <div>
                      Net take-home:{" "}
                      <span className="text-foreground tabular-nums">
                        {fmt(nodeTip.meta.net ?? 0)}
                      </span>
                    </div>
                  </div>
                )}
              {nodeTip.kind === "category" && (
                <div className="mt-1 pt-1 text-[11px]">
      ...
      <AlertDialog
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setConfirmNav(null)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
