...
import { getUserIncomes } from "@/actions/user"
...
export function TotalIncomeCard({
    ...
    (targetMonth: Date) => {
      ...
      incomes.forEach((income) => {
        ...
        if (appliesThisMonth) {
          ...
          if (income.subjectToCpf && income.employeeCpfContribution) {
            const monthlyCpfDeduction = parseFloat(
  ...
  const monthChange = useMemo(() => {
    ...
    const percentChange =
      previousMonthData.net > 0 ? (change / previousMonthData.net) * 100 : 0
  ...
  const hasCpfDeductions =
    incomes.length > 0 && currentMonthData.cpfDeduction > 0
        ...
        <CardContent>
          ...
          {incomes.length > 0 && monthChange.amount !== 0 && (
            <div
              ...
              <span>
                {monthChange.amount > 0 ? "+" : ""}$
                {Math.round(Math.abs(monthChange.amount)).toLocaleString()}
                {monthChange.percent !== 0 &&
                  ` (${monthChange.percent > 0 ? "+" : ""}${monthChange.percent.toFixed(1)}%)`}
              </span>
