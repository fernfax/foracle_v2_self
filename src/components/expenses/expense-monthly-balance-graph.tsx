...
import {
  ...
  type SpecialItem
} from "@/lib/finance/balance-calculator"
...
const CustomBalanceDot = (props: CustomDotProps) => {
  ...
  if (payload.specialItems && payload.specialItems.length > 0) {
    ...
    const uniqueTypes = new Set(
      payload.specialItems.map((item: SpecialItem) => item.type)
    )
        ...
        <div className="bg-card border-border rounded-lg border p-3 shadow-lg">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">
              {getSpecialItemLabel(arrowData.type)}: $
              {arrowData.amount.toLocaleString()} - {arrowData.name}
            </span>
          </div>
        </div>
      )
...
export function ExpenseMonthlyBalanceGraph({
  ...
  const balanceData = useMemo(() => {
    return baseBalanceData.map((data, index) => ({
      ...
      balanceWithInvestments:
        includeInvestments && investmentProjections
          ? data.balance + (investmentProjections[index] || 0)
          : undefined,
      // For non-cumulative view, add monthly balance with investment growth
      monthlyBalanceWithInvestments:
        includeInvestments && investmentProjections
          ? data.monthlyBalance +
            (index > 0
              ? (investmentProjections[index] || 0) -
                (investmentProjections[index - 1] || 0)
              : 0)
          : undefined
    }))
  ...
  const finalBalanceWithInvestments =
    includeInvestments && balanceData.length > 0
  ...
  const allBalanceValues =
    viewMode === "cumulative"
      ? [
          ...balanceData.map((d) => d.balance),
          ...(includeInvestments
            ? balanceData.map((d) => d.balanceWithInvestments || d.balance)
            : [])
        ]
      : [
          ...
          ...(includeInvestments
            ? balanceData.map(
                (d) => d.monthlyBalanceWithInvestments || d.monthlyBalance
              )
            : [])
        ]
  ...
  return (
    <Wrapper>
      <CardHeader className={embedded ? "pt-0 pb-2" : "pt-4 pb-2"}>
        ...
        {!embedded && (
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent
                      ...
                      <p>
                        ...
                        <strong>All Active Income</strong> (including one-time
                        income) and <strong>All Active Expenses</strong>{" "}
                        (including one-time expenses). Future income milestones
                        are only included for incomes with &quot;Account for
                        Future Change&quot; enabled. Starting balance is based
                        on the sum of all current holdings.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
            ...
            <div
              className="grid grid-cols-2 gap-2 sm:flex sm:gap-4"
              data-tour="graph-controls">
              <div className="space-y-1 sm:min-w-[180px] sm:space-y-2">
                <Label
                  htmlFor="viewMode"
                ...
                <Select
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="non-cumulative">
                      Non-Cumulative
                    </SelectItem>
                  </SelectContent>
                </Select>
        ...
        <div
          ...
          {embedded && (
            <div className="grid grid-cols-2 gap-2 sm:flex sm:shrink-0 sm:gap-3">
              <div className="bg-muted flex min-h-[64px] flex-col justify-between rounded-lg p-2 sm:w-36 sm:p-3">
                <Label
                  htmlFor="viewModeEmbedded"
                  className="text-foreground mb-0.5 text-xs font-normal">
                  View Mode
                </Label>
                <Select
                  value={viewMode}
                  ...
                  <SelectTrigger
                    id="viewModeEmbedded"
                    className="bg-background h-7 w-full text-xs sm:h-8 sm:text-sm">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="cumulative">Cumulative</SelectItem>
          ...
          <div className="bg-muted flex-1 rounded-lg p-3 sm:p-4">
            <div className="flex h-full items-center justify-between gap-2 sm:gap-4">
              <div className="min-w-0">
                <p className="text-foreground mb-0.5 text-xs">
                  Starting Balance
                </p>
                <p className="text-foreground text-base font-semibold tabular-nums sm:text-xl">
              ...
              <div className="flex min-w-[76px] flex-1 flex-col items-center justify-center px-1">
                <span
                ...
                <div
                  className="text-muted-foreground/50 flex w-full items-center"
                  aria-hidden>
                  <span className="h-px flex-1 bg-current" />
                  <ArrowRight className="-ml-1 h-3.5 w-3.5 shrink-0 sm:h-4 sm:w-4" />
                </div>
                <span className="text-muted-foreground mt-0.5 text-[9px] sm:text-[10px]">
                  Net change
                </span>
                {includeInvestments && (
      ...
      <CardContent className="pt-2">
        ...
        <div
          className={`relative ${embedded ? "h-[340px] w-full sm:h-[calc(90vh-198px)] sm:min-h-[400px]" : "h-[350px] w-full"}`}>
