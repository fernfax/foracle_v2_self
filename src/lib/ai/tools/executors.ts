...
async function executeGetIncomeSummary(
  ...
  for (const income of userIncomes) {
    ...
    const monthlyGross = isUsingHistorical
      ? grossAmount
      : grossAmount * frequencyMultiplier
    ...
    if ((isUsingMilestone || isUsingHistorical) && subjectToCpf) {
      ...
      const totalCpf =
        cpfResult.employeeCpfContribution + cpfResult.employerCpfContribution
      ...
      employeeCpf = income.employeeCpfContribution
        ? parseFloat(income.employeeCpfContribution)
        : 0
      employerCpf = income.employerCpfContribution
        ? parseFloat(income.employerCpfContribution)
        : 0
      netTakeHome = income.netTakeHome
        ? parseFloat(income.netTakeHome)
        : monthlyGross
      cpfOA = income.cpfOrdinaryAccount
        ? parseFloat(income.cpfOrdinaryAccount)
        : 0
      cpfSA = income.cpfSpecialAccount
        ? parseFloat(income.cpfSpecialAccount)
        : 0
      cpfMA = income.cpfMedisaveAccount
        ? parseFloat(income.cpfMedisaveAccount)
        : 0
    }
    ...
    const cpfBreakdown: CpfBreakdown | null = subjectToCpf
      ? {
          ...
          totalContribution:
            Math.round((employeeCpf + employerCpf) * 100) / 100,
    ...
    incomeSources.push({
      ...
      monthlyAmount:
        Math.round((subjectToCpf ? netTakeHome : monthlyGross) * 100) / 100,
  ...
  return {
    ...
    totalCpfContribution:
      Math.round((totalEmployeeCpf + totalEmployerCpf) * 100) / 100,
    // CPF account totals
...
async function executeGetExpensesSummary(
  ...
  for (const expense of userExpenses) {
    ...
    const expenseStartDate = expense.startDate
  ...
  const categoryBreakdown: ExpenseCategoryBreakdown[] = Object.entries(
    categoryTotals
  )
  ...
  const availableCategories: ExpenseCategoryInfo[] = userExpenseCategories.map(
    (cat) => ({
      id: cat.id,
      name: cat.name,
      icon: cat.icon,
...
async function executeGetFamilySummary(
  ...
  for (const member of userFamilyMembers) {
    ...
    const incomeIdentityHint =
      memberIncomes.length > 0 ? memberIncomes[0].name : null
    ...
    if (member.isContributing) {
      ...
      notes.push(
        `${member.name} is excluded from household income totals per family settings`
      )
    }
    ...
    for (const income of memberIncomes) {
      if (income.futureMilestones) {
        try {
          const milestones: FutureMilestone[] = JSON.parse(
    ...
    if (effectiveScope === "member") {
      const nameMatch = params.memberName
        ? member.name.toLowerCase().includes(params.memberName.toLowerCase()) ||
          (params.memberName.toLowerCase() === "spouse" &&
            member.relationship?.toLowerCase() === "spouse") ||
          (params.memberName.toLowerCase() === "wife" &&
            member.relationship?.toLowerCase() === "spouse") ||
          (params.memberName.toLowerCase() === "husband" &&
            member.relationship?.toLowerCase() === "spouse")
        : false
  ...
  if (householdMembers.length === 0) {
    notes.push(
      "No family members have been added yet. Income totals include all user incomes."
    )
  }
  ...
  if (
    ...
    notes.push(
      `Could not find family member matching '${params.memberName || params.memberId}'. Showing all household members.`
    )
  }
  ...
  for (const income of userIncomes) {
    ...
    if (income.subjectToCpf) {
      if (isUsingMilestone) {
        ...
        effectiveAmount = income.netTakeHome
          ? parseFloat(income.netTakeHome)
          : amount
      }
    }
    ...
    if (frequency === "monthly") {
      ...
      if (incomeStartDate) {
        const monthsSinceStart =
          (year - incomeStartDate.getFullYear()) * 12 +
...
function calculateMonthlyExpensesForMonth(
  ...
  for (const expense of userExpenses) {
    ...
    if (frequency === "monthly") {
      ...
      if (expenseStartDate) {
        const monthsSinceStart =
          (year - expenseStartDate.getFullYear()) * 12 +
...
async function executeGetBalanceSummary(
  ...
  if (holdings.length === 0) {
    ...
    notes.push(
      `Starting balance calculated from ${holdings.length} holding(s).`
    )
  }
  ...
  assumptions.push(
    "Future income estimated using current active income sources"
  )
  assumptions.push(
    "Recurring expenses assumed constant unless end date specified"
  )
  ...
  const usingNewHypotheticalsFormat =
    params.hypotheticals && params.hypotheticals.length > 0

  if (usingNewHypotheticalsFormat) {
    effectiveHypotheticals = params.hypotheticals!
    assumptions.push(
      `Using ${effectiveHypotheticals.length} hypothetical scenario(s)`
    )
  } else {
  ...
  for (let i = 0; i < monthCount; i++) {
    ...
    const appliedHypotheticals: Array<{
      type: "income" | "expense"
      amount: number
      label?: string
    }> = []
  ...
  const firstExpenseHyp = effectiveHypotheticals.find(
    (h) => h.type === "expense"
  )
  ...
  if (firstExpenseHyp) {
    ...
    const targetProjection = monthlyProjections.find(
      (p) => p.month === firstExpenseHyp.month
    )
    const monthNetWithout = targetProjection
      ? targetProjection.netBalance + firstExpenseHyp.amount
      : null
    ...
    hypotheticalImpact = {
      ...
      percentOfMonthlyBalance:
        monthNetWithout && monthNetWithout !== 0
          ? Math.round((firstExpenseHyp.amount / monthNetWithout) * 1000) / 10
          : null
    }
    notes.push(
      `Hypothetical expense of $${firstExpenseHyp.amount.toLocaleString()}${firstExpenseHyp.label ? ` (${firstExpenseHyp.label})` : ""} in ${formatMonthLabel(firstExpenseHyp.month)} included.`
    )
    ...
    notes.push(
      `Hypothetical income of $${firstIncomeHyp.amount.toLocaleString()}${firstIncomeHyp.label ? ` (${firstIncomeHyp.label})` : ""} in ${formatMonthLabel(firstIncomeHyp.month)} included.`
    )
  }
  ...
  if (effectiveHypotheticals.length > 0) {
    ...
    const totalHypExpense = effectiveHypotheticals
      ...
      .reduce((sum, h) => sum + h.amount, 0)
    const affectedMonths = new Set(effectiveHypotheticals.map((h) => h.month))
      .size
  ...
  if (minEndBalanceProvided || minMonthlyBalanceProvided) {
    const minEndBalanceBreached =
      minEndBalanceProvided && cumulativeBalance < params.minEndBalance!
    ...
    if (minEndBalanceBreached) {
      notes.push(
        `Warning: Final balance ($${Math.round(cumulativeBalance).toLocaleString()}) is below minimum required ($${params.minEndBalance!.toLocaleString()}).`
      )
    }
    if (minMonthlyBalanceBreached && firstBreachMonth) {
      notes.push(
        `Warning: Balance drops below minimum ($${params.minMonthlyBalance!.toLocaleString()}) starting in ${formatMonthLabel(firstBreachMonth)}.`
      )
    }
  }
  ...
  if (params.computeMaxAffordableExpenseMonth) {
    ...
    const baselineProjections: Array<{
      month: string
    ...
    const targetMonthIndex = baselineProjections.findIndex(
    ...
    if (targetMonthIndex === -1) {
      ...
      affordabilityAnalysis = {
        ...
        assumptions: [
          `Target month ${targetMonth} is outside the projection range (${params.fromMonth} to ${params.toMonth})`
        ]
      }
    } else {
      ...
      const maxAffordable = Math.max(
        0,
        minBalanceFromTarget - minAllowedBalance
      )
      ...
      if (usingSafetyFloor) {
        notes.push(
          `Max affordable one-time expense in ${formatMonthLabel(targetMonth)}: $${Math.round(maxAffordable).toLocaleString()} (while maintaining 6-month emergency fund)`
        )
      } else {
        notes.push(
          `Max affordable one-time expense in ${formatMonthLabel(targetMonth)}: $${Math.round(maxAffordable).toLocaleString()}`
        )
      }
    }
  }
  ...
  const hypotheticalExpenses = effectiveHypotheticals.filter(
    (h) => h.type === "expense"
  )
  ...
  if (hypotheticalExpenses.length > 0) {
    ...
    const expenseMonthProjection = monthlyProjections.find(
      (p) => p.month === expenseMonth
    )
  ...
  const emergencyFundMonths =
    baseMonthlyNetIncome > 0 ? safetyCheckBalance / baseMonthlyNetIncome : 0
  ...
  if (baseMonthlyNetIncome === 0) {
    ...
    statusLabel = "Unknown"
    recommendation =
      "Unable to calculate emergency fund coverage - no recurring income data found."
  ...
  if (safetyStatus !== "green") {
    notes.push(
      `Safety Alert: Balance reaches ${emergencyFundMonths.toFixed(1)} months of income (${statusLabel}) in ${formatMonthLabel(safetyCheckMonth)}.`
    )
  }
  ...
  if (params.findSafeMonthForExpense) {
    ...
    for (let i = 0; i < baselineProjections.length; i++) {
      ...
      if (balanceAfter >= safetyThreshold) {
        ...
        emergencyFundMonthsAfter =
          baseMonthlyNetIncome > 0 ? balanceAfter / baseMonthlyNetIncome : 0
        monthsToWait = i
        isSafeNow = i === 0
        break
      }
    ...
    safePurchaseRecommendation = {
      ...
      recommendedMonth,
      recommendedMonthLabel: recommendedMonth
        ? formatMonthLabel(recommendedMonth)
        : null,
      balanceAfterPurchase: balanceAfterPurchase
        ? Math.round(balanceAfterPurchase * 100) / 100
        : null,
      emergencyFundMonthsAfter: emergencyFundMonthsAfter
        ? Math.round(emergencyFundMonthsAfter * 10) / 10
        : null,
      monthsToWait,
      isSafeNow,
...
async function executeGetHoldingsSummary(
  ...
  const memberTotals: Record<
    string,
    { totalAmount: number; accountCount: number }
  > = {}
  ...
  if (holdings.length === 0) {
    notes.push(
      "No holdings found. Consider adding your bank accounts to track your liquid assets."
    )
  } else {
...
async function executeGetPropertyAssetsSummary(
  ...
  const properties = await db
    ...
    .where(
      and(
        eq(propertyAssets.familyId, familyId),
  ...
  for (const property of properties) {
    ...
    const loanTaken = property.loanAmountTaken
      ? parseFloat(property.loanAmountTaken)
      : null
    ...
    propertyItems.push({
      ...
      principalCpfWithdrawn: property.principalCpfWithdrawn
        ? parseFloat(property.principalCpfWithdrawn)
        : null,
      housingGrantTaken: property.housingGrantTaken
        ? parseFloat(property.housingGrantTaken)
        : null,
      accruedInterestToDate: property.accruedInterestToDate
        ? parseFloat(property.accruedInterestToDate)
        : null,
  ...
  propertyItems.sort(
    (a, b) => b.originalPurchasePrice - a.originalPurchasePrice
  )
  ...
  if (properties.length === 0) {
    ...
    if (totalOutstandingLoans > 0) {
      const overallLoanProgress =
        Math.round((totalEquity / totalPropertyValue) * 1000) / 10
...
async function executeGetVehicleAssetsSummary(
  ...
  const vehicles = await db
    ...
    .where(
      and(
        eq(vehicleAssets.familyId, familyId),
        eq(vehicleAssets.isActive, true)
      )
    )
  ...
  for (const vehicle of vehicles) {
    ...
    const loanTaken = vehicle.loanAmountTaken
      ? parseFloat(vehicle.loanAmountTaken)
      : null
    const loanRepaid = vehicle.loanAmountRepaid
      ? parseFloat(vehicle.loanAmountRepaid)
      : null
    const monthlyPayment = vehicle.monthlyLoanPayment
      ? parseFloat(vehicle.monthlyLoanPayment)
      : null
    ...
    if (loanTaken !== null) {
      ...
      loanProgress =
        loanTaken > 0
          ? Math.round(((loanRepaid || 0) / loanTaken) * 1000) / 10
          : null
    }
    ...
    if (vehicle.coeExpiryDate) {
      ...
      coeYearsRemaining =
        Math.round((diffMs / (1000 * 60 * 60 * 24 * 365)) * 10) / 10
    ...
    vehicleItems.push({
      ...
      outstandingLoan:
        outstandingLoan !== null
          ? Math.round(outstandingLoan * 100) / 100
          : null,
      loanProgress,
      coeYearsRemaining,
  ...
  if (vehicles.length === 0) {
    ...
    const expiringCOE = vehicleItems.filter(
      (v) => v.coeYearsRemaining !== null && v.coeYearsRemaining <= 2
    )
    if (expiringCOE.length > 0) {
      notes.push(
        `${expiringCOE.length} vehicle(s) with COE expiring within 2 years.`
      )
    }
  }
...
async function executeGetOtherAssetsSummary(
  ...
  if (appliedFilter) {
    userAssets = userAssets.filter(
      (a) => a.type.toLowerCase() === appliedFilter.toLowerCase()
    )
  }
  ...
  if (userAssets.length === 0) {
    ...
    if (totalPurchaseValue > 0) {
      const overallGainLossPercent =
        Math.round((totalGainLoss / totalPurchaseValue) * 1000) / 10
      if (totalGainLoss >= 0) {
        notes.push(
          `Overall gain: $${totalGainLoss.toLocaleString()} (+${overallGainLossPercent}%).`
        )
      } else {
        notes.push(
          `Overall loss: $${Math.abs(totalGainLoss).toLocaleString()} (${overallGainLossPercent}%).`
        )
      }
    }
  }
...
async function executeGetInsuranceSummary(
  ...
  if (typeFilter) {
    userPolicies = userPolicies.filter(
      (p) => p.policyType.toLowerCase() === typeFilter.toLowerCase()
    )
  }
  ...
  if (userPolicies.length === 0) {
    ...
    if (totalDeathCoverage > 0) {
      notes.push(
        `Total death/TPD coverage: $${totalDeathCoverage.toLocaleString()}.`
      )
    }
    if (totalCriticalIllnessCoverage > 0) {
      notes.push(
        `Total critical illness coverage: $${totalCriticalIllnessCoverage.toLocaleString()}.`
      )
    }
  }
  ...
  return {
    ...
    totalCriticalIllnessCoverage:
      Math.round(totalCriticalIllnessCoverage * 100) / 100,
    policiesByType,
    policiesByProvider,
...
async function executeGetDailyExpenseSummary(
  ...
  if (params.subcategoryName) {
    filteredExpenses = filteredExpenses.filter(
      (e) =>
        ...
        params.subcategoryName!.toLowerCase()
    )
  }
  ...
  for (const expense of filteredExpenses) {
    ...
    expenseItems.push({
      ...
      originalAmount: expense.originalAmount
        ? parseFloat(expense.originalAmount)
        : null,
  ...
  const categoryBreakdown: DailyExpenseCategoryBreakdown[] = Object.entries(
    categoryMap
  )
    .map(([catName, data]) => ({
      categoryName: catName,
      ...
      percentOfTotal:
        totalSpent > 0 ? (data.totalAmount / totalSpent) * 100 : 0,
  ...
  const availableCategories = [
    ...new Set(userExpenseCategories.map((c) => c.name))
  ]
  ...
  try {
    ...
    switch (toolName as ToolName) {
      case "get_income_summary":
        data = await executeGetIncomeSummary(
          validationResult.data as MonthParams,
          familyId
        )
        break

      case "get_expenses_summary":
        data = await executeGetExpensesSummary(
          validationResult.data as MonthParams,
          familyId
        )
        break
      ...
      case "get_family_summary":
        data = await executeGetFamilySummary(
          validationResult.data as FamilySummaryParams,
          familyId
        )
        break

      case "get_balance_summary":
        data = await executeGetBalanceSummary(
          validationResult.data as BalanceSummaryParams,
          familyId
        )
        break
      ...
      case "get_other_assets_summary":
        data = await executeGetOtherAssetsSummary(
          validationResult.data as OtherAssetsSummaryParams,
          familyId
        )
        break

      case "get_insurance_summary":
        data = await executeGetInsuranceSummary(
          validationResult.data as InsuranceSummaryParams,
          familyId
        )
        break

      case "get_daily_expense_summary":
        data = await executeGetDailyExpenseSummary(
          validationResult.data as DailyExpenseSummaryParams,
          familyId
        )
        break

      case "search_knowledge":
        data = await executeSearchKnowledge(
          validationResult.data as SearchKnowledgeParams
        )
        break
