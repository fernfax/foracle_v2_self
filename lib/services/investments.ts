...
export async function updateInvestment(
  ...
  if (patch.currentCapital !== undefined) update.currentCapital = patch.currentCapital;
  if (patch.projectedYield !== undefined) update.projectedYield = patch.projectedYield;
...
export async function getInvestmentsSummary(
  ...
  const weighted = active.reduce(
    (acc, r) => acc + parseFloat(r.currentCapital) * parseFloat(r.projectedYield),
    0
  );
  const averageYield = totalPortfolioValue > 0 ? weighted / totalPortfolioValue : 0;
