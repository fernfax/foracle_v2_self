...
export async function updateDailyExpense(
  ...
  if (patch.categoryId !== undefined)
    update.categoryId = patch.categoryId ?? null
  if (patch.categoryName !== undefined) update.categoryName = patch.categoryName
  if (patch.subcategoryId !== undefined)
    update.subcategoryId = patch.subcategoryId ?? null
  if (patch.subcategoryName !== undefined)
    update.subcategoryName = patch.subcategoryName ?? null
  if (patch.amount !== undefined) update.amount = patch.amount
  if (patch.note !== undefined) update.note = patch.note ?? null
  if (patch.date !== undefined) update.date = patch.date
  if (patch.originalCurrency !== undefined)
    update.originalCurrency = patch.originalCurrency ?? null
  if (patch.originalAmount !== undefined)
    update.originalAmount = patch.originalAmount ?? null
  if (patch.exchangeRate !== undefined)
    update.exchangeRate = patch.exchangeRate ?? null

  const [row] = await db
    .update(dailyExpenses)
    .set(update)
    .where(
      and(eq(dailyExpenses.id, id), eq(dailyExpenses.familyId, ctx.familyId))
    )
    .returning()
  return row
}
