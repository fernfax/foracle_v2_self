...
export async function buildDiagram(): Promise<Diagram> {
  ...
  const sourcesToScan: Array<{ nodeId: string; absPath: string }> = [
    ...pages.map((p) => ({ nodeId: p.id, absPath: path.join(PROJECT_ROOT, p.filePath!) })),
    ...apis.map((a) => ({ nodeId: a.id, absPath: path.join(PROJECT_ROOT, a.filePath!) })),
    ...actions.map((a) => ({ nodeId: a.nodeId, absPath: path.join(PROJECT_ROOT, a.filePath) })),
  ];
...
async function discoverTables(): Promise<TableMeta[]> {
  ...
  const regex = /^\s*export\s+const\s+(\w+)\s*=\s*pgTable\s*\(\s*["']([^"']+)["']/gm;
  const out: TableMeta[] = [];
...
async function discoverPages(): Promise<PageMeta[]> {
  ...
  await walk(path.join(PROJECT_ROOT, "app"), async (absPath) => {
    if (!absPath.endsWith("/page.tsx") && !absPath.endsWith("\\page.tsx")) return;
...
async function discoverApiRoutes(): Promise<ApiMeta[]> {
  ...
  await walk(apiRoot, async (absPath) => {
    if (!absPath.endsWith("/route.ts") && !absPath.endsWith("\\route.ts")) return;
    const rel = path.relative(PROJECT_ROOT, absPath);
    const route = "/" + rel
      .replace(/^app\//, "")
      .replace(/\/route\.ts$/, "");
