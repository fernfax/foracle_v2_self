...
describe("ToolRegistry", () => {
  ...
  describe("validateArgs", () => {
    ...
    it("should throw for unknown tool", () => {
      ...
      expect(() => registry.validateArgs("unknown" as ToolName, {})).toThrow(
        "Unknown tool"
      )
    })
  })
...
describe("Zod Schemas", () => {
  describe("MonthParamSchema", () => {
    it("should accept valid YYYY-MM format", () => {
      expect(MonthParamSchema.safeParse({ month: "2025-01" }).success).toBe(
        true
      )
      expect(MonthParamSchema.safeParse({ month: "2025-12" }).success).toBe(
        true
      )
      expect(MonthParamSchema.safeParse({ month: "1999-06" }).success).toBe(
        true
      )
    })

    it("should reject invalid month values", () => {
      expect(MonthParamSchema.safeParse({ month: "2025-00" }).success).toBe(
        false
      )
      expect(MonthParamSchema.safeParse({ month: "2025-13" }).success).toBe(
        false
      )
      expect(MonthParamSchema.safeParse({ month: "2025-1" }).success).toBe(
        false
      )
    })
  })
  ...
  describe("FamilySummaryParamSchema", () => {
    it("should accept valid scope values", () => {
      expect(
        FamilySummaryParamSchema.safeParse({ scope: "household" }).success
      ).toBe(true)
      expect(
        FamilySummaryParamSchema.safeParse({ scope: "member" }).success
      ).toBe(true)
      expect(
        FamilySummaryParamSchema.safeParse({ scope: "auto" }).success
      ).toBe(true)
    })
    ...
    describe("v2 multi-hypotheticals", () => {
      ...
      it("should accept hypotheticals without labels", () => {
        const result = BalanceSummaryParamSchema.safeParse({
          fromMonth: "2025-01",
          toMonth: "2025-06",
          hypotheticals: [{ type: "expense", amount: 500, month: "2025-02" }]
        })
        expect(result.success).toBe(true)
        if (result.success) {
      ...
      it("should reject hypothetical with invalid type", () => {
        const result = BalanceSummaryParamSchema.safeParse({
          fromMonth: "2025-01",
          toMonth: "2025-12",
          hypotheticals: [{ type: "transfer", amount: 1000, month: "2025-03" }]
        })
        expect(result.success).toBe(false)
      })
      ...
      it("should reject hypothetical with negative amount", () => {
        const result = BalanceSummaryParamSchema.safeParse({
          fromMonth: "2025-01",
          toMonth: "2025-12",
          hypotheticals: [{ type: "expense", amount: -100, month: "2025-03" }]
        })
        expect(result.success).toBe(false)
      })
      ...
      it("should reject hypothetical with invalid month format", () => {
        const result = BalanceSummaryParamSchema.safeParse({
          fromMonth: "2025-01",
          toMonth: "2025-12",
          hypotheticals: [{ type: "expense", amount: 100, month: "March 2025" }]
        })
        expect(result.success).toBe(false)
      })
    })
    ...
    describe("v2 backwards compatibility", () => {
      ...
      it("should accept both legacy and new params (new takes precedence at runtime)", () => {
        const result = BalanceSummaryParamSchema.safeParse({
          ...
          hypotheticalExpenseMonth: "2025-06",
          // New params
          hypotheticals: [{ type: "expense", amount: 2000, month: "2025-03" }]
        })
        expect(result.success).toBe(true)
        if (result.success) {
  ...
  describe("HypotheticalItemSchema", () => {
    ...
    it("should reject missing required fields", () => {
      expect(
        HypotheticalItemSchema.safeParse({ type: "expense" }).success
      ).toBe(false)
      expect(HypotheticalItemSchema.safeParse({ amount: 1000 }).success).toBe(
        false
      )
      expect(
        HypotheticalItemSchema.safeParse({ month: "2025-06" }).success
      ).toBe(false)
    })
  })
})
