# Foracle Knowledge Base

Foracle's AI Assistant comes with built-in financial knowledge to help answer your questions. This page explains what topics the assistant knows about.

---

## What is the Knowledge Base?

The Knowledge Base is a collection of financial information that Foracle's AI Assistant can search through when you ask questions. Think of it as a "brain" of financial knowledge that helps the assistant give you accurate, helpful answers.

When you ask a general financial question, the assistant searches this knowledge base to find relevant information before responding.

---

## Topics Covered

### 1. CPF (Central Provident Fund) - Updated January 2026

Comprehensive knowledge about Singapore's mandatory savings scheme, including the latest 2026 updates:

#### CPF Ordinary Wage Ceiling

- **Current ceiling: $8,000/month** (effective 1 Jan 2026)
- History of ceiling increases from $6,000 (2016) to $8,000 (2026)
- Annual salary ceiling: $102,000
- CPF Annual Limit: $37,740
- How the ceiling affects high earners

#### CPF Contribution Rates (2026)

- **Age 55 and below**: 37% (17% employer + 20% employee)
- **Age 55-60**: 34% (16% employer + 18% employee) - _increased in 2026_
- **Age 60-65**: 25% (12.5% employer + 12.5% employee) - _increased in 2026_
- **Age 65-70**: 16.5% (9% employer + 7.5% employee)
- **Above 70**: 12.5% (7.5% employer + 5% employee)

#### CPF Account Types & Allocation

- **Ordinary Account (OA)** - Housing, insurance, investment, education (2.5% interest)
- **Special Account (SA)** - Retirement savings (4% interest)
- **MediSave Account (MA)** - Healthcare expenses (4% interest)
- **Retirement Account (RA)** - Created at age 55 (up to 6% interest)
- Detailed allocation rates by age group

**Example questions you can ask:**

- "What is the CPF OW ceiling in 2026?"
- "How much CPF do I contribute if I'm 58 years old?"
- "What are the CPF account types?"
- "How is my CPF split between OA, SA, and MA?"
- "What interest does my CPF earn?"
- "What changed for CPF in 2026?"

---

### 2. Emergency Fund Guidelines

Practical advice on building and maintaining your financial safety net:

- **How Much to Save** - Recommendations from financial experts:
  - Minimum: 3-6 months of expenses
  - Comfortable: 6-9 months of expenses
  - Conservative: 9-12 months of expenses
- **Foracle's Safety Assessment** - How we evaluate your financial health:
  - 🟢 **Green (Safe)**: 9+ months of net income saved
  - 🟡 **Yellow (Caution)**: 6-9 months saved
  - 🔴 **Red (At Risk)**: Less than 6 months saved
- **What to Include** - Expenses your emergency fund should cover
- **Where to Keep It** - Best places to store your emergency fund in Singapore (high-yield savings, SSB, fixed deposits)

**Example questions you can ask:**

- "How much emergency fund should I have?"
- "What does the green/yellow/red safety status mean?"
- "Where should I keep my emergency fund?"

---

### 3. Budgeting Basics

Fundamental concepts to help you manage your money:

- **The 50/30/20 Rule** - A popular budgeting framework:
  - 50% for Needs (housing, food, transport)
  - 30% for Wants (entertainment, dining out)
  - 20% for Savings (emergency fund, retirement, investments)
- **Expense Categories**:
  - Fixed expenses (rent, insurance, subscriptions)
  - Variable expenses (groceries, utilities, transport)
  - Periodic expenses (annual fees, maintenance, gifts)
- **Budget Tracking Tips** - Best practices for staying on top of your finances

**Example questions you can ask:**

- "What is the 50/30/20 rule?"
- "What are fixed vs variable expenses?"
- "How should I track my budget?"

---

## What the Assistant Knows vs. Your Personal Data

| Type                   | Source               | Examples                                                 |
| ---------------------- | -------------------- | -------------------------------------------------------- |
| **General Knowledge**  | Knowledge Base       | "What is CPF?", "How much emergency fund should I have?" |
| **Your Personal Data** | Your Foracle Account | "What's my income this month?", "How much did I spend?"  |

The assistant automatically knows when to use the knowledge base (for general questions) vs. your personal data (for questions about your finances).

---

## Coming Soon

We're continuously expanding the knowledge base. Future topics may include:

- Investment basics (stocks, bonds, ETFs)
- Insurance types in Singapore
- Tax planning tips
- Retirement planning
- Home buying guide (BTO, resale, private)

---

## Have a Question Not Covered?

If you ask about a topic not in the knowledge base, the assistant will let you know. Feel free to share feedback on what topics you'd like us to add!
