...
const NODE_COLOR: Record<DiagramNodeKind, { bg: string; border: string; text: string }> = {
...
const LEGEND_ENTRIES: Array<{ kind: DiagramNodeKind; label: string; hint: string }> = [
  { kind: "page", label: "Page", hint: "App router route (app/.../page.tsx)" },
  { kind: "api", label: "API route", hint: "app/api/.../route.ts" },
  { kind: "action", label: "Server action", hint: "lib/actions/*.ts exported function" },
...
export function DiagramCanvas({ diagram }: DiagramCanvasProps) {
  ...
  return (
    <div className="relative h-[calc(100vh-160px)] w-full overflow-hidden rounded-md border border-border/40 bg-card">
      <ReactFlow
        nodes={styledNodes}
        edges={styledEdges}
        onNodeClick={(_, n) => {
...
function Legend() {
  return (
    <div className="absolute left-3 top-3 z-10 rounded-md border border-border/40 bg-background/95 px-3 py-2 shadow-sm backdrop-blur">
      ...
      <ul className="space-y-0.5">
        {nodes.map((n) => (
          <li key={n.id} className="flex items-center gap-1.5 text-[11px]">
            ...
            />
            <span className="truncate font-mono text-foreground/80" title={n.label}>
              {n.label}
            </span>
          </li>
        ))}
...
function buildLayout(diagram: Diagram): { rfNodes: Node[]; rfEdges: Edge[] } {
  ...
  const pageColumns = new Map<string, { page: DiagramNode; members: DiagramNode[] }>();
  ...
  type Col = { id: string; label: string; head?: DiagramNode; members: DiagramNode[] };
  const columns: Col[] = [];
  for (const p of pages) {
    const col = pageColumns.get(p.id)!;
    columns.push({ id: p.id, label: p.label, head: col.page, members: col.members });
  }
  if (sharedMembers.length > 0) {
    columns.push({ id: "__shared", label: SHARED_COL_LABEL, members: sharedMembers });
  }
