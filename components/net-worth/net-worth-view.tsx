...
import { deleteCurrentHolding, type CurrentHolding } from "@/lib/actions/current-holdings";
...
const CHIP_ACCENT: Record<AssetClassKey, "brand" | "jungle" | "teal" | "gold" | "neutral"> = {
  cash: "teal",
  property: "jungle",
  vehicle: "brand",
  investments: "brand",
...
export function NetWorthView({ summary, initialHoldings }: NetWorthViewProps) {
  ...
  const hasAnything = summary.assetRows.length > 0 || summary.liabilityRows.length > 0;
      ...
      {!hasAnything ? (
        ...
        <>
          ...
          <SectionCard
            ...
            noBodyPadding
          >
            <RowList rows={summary.assetRows} onEdit={openEditCash} onDelete={setDeleteId} />
          </SectionCard>
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently deletes the bank holding record. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
          ...
          <div className="flex w-[76px] shrink-0 justify-end">
            {row.editable && onEdit && onDelete ? (
              ...
              <Button
                ...
                variant="ghost"
                size="icon-sm"
                className="border-transparent text-muted-foreground hover:border-transparent hover:text-primary"
                title="Manage"
              >
