        ...
        if (income.futureMilestones) {
          try {
            ...
            const applicableMilestones = milestones
              .filter(
                (m: { targetMonth: string }) => m.targetMonth <= targetPeriod
              )
              .sort((a: { targetMonth: string }, b: { targetMonth: string }) =>
    ...
    (targetMonth: Date) => {
      ...
      expenses.forEach((expense) => {
        ...
        const startDate = expense.startDate
          ? parseLocalDate(expense.startDate)
          : null
  ...
  const monthChange = useMemo(() => {
    ...
    const percentChange =
      previousNetSavings !== 0
        ? (change / Math.abs(previousNetSavings)) * 100
        : 0
