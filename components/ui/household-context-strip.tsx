...
export function HouseholdContextStrip({ summary, tab }: HouseholdContextStripProps) {
  const { grossIncome, netIncome, cpfEmployeeTotal, monthlyExpenses, surplus, liquidHoldings, runwayMonths, memberCount } = summary;

  // Empty state: no incomes at all
  if (grossIncome === 0 && tab !== "family" && tab !== "holdings") {
    return (
      <div className="mb-4 rounded-md border border-border/40 bg-muted/40 px-3 py-2 text-[11px]">
        <Link href="/user?tab=incomes" className="text-muted-foreground hover:text-foreground underline-offset-2 hover:underline">
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-x-2 gap-y-1 rounded-md border border-border/40 bg-muted/30 px-3 py-1.5 text-[11px]">
      {content}
