...
describe("engine ↔ constants", () => {
  ...
  it("getCPFAllocationByAge returns the constant allocations for a sample of bands", () => {
    expect(getCPFAllocationByAge(30)).toEqual(
      CPF_ALLOCATION_RATES["35_and_below"]
    )
    expect(getCPFAllocationByAge(63)).toEqual(
      CPF_ALLOCATION_RATES["above_60_to_65"]
    )
  ...
  it("CPF_ALLOCATION_BRACKETS are derived from CPF_ALLOCATION_RATES", () => {
    CPF_ALLOCATION_BAND_ORDER.forEach((band) => {
      ...
      if (band === "35_and_below") {
        expect(CPF_ALLOCATION_BRACKETS[0]).toMatchObject(
          CPF_ALLOCATION_RATES["35_and_below"]
        )
      }
    ...
    CPF_ALLOCATION_BAND_ORDER.forEach((band, i) => {
      expect(CPF_ALLOCATION_BRACKETS[i]).toMatchObject(
        CPF_ALLOCATION_RATES[band]
      )
    })
  })
})
