...
async function pickDob(page: Page, year: number, monthShort: string, day: number) {
  ...
  await cal.locator("button").filter({ hasText: /^[A-Za-z]{3,9} \d{4}$/ }).first().click();
  // months view header "YYYY" -> years view
  await cal.locator("button").filter({ hasText: /^\d{4}$/ }).first().click();
...
test(`onboarding walkthrough — scenario ${KEY} (${S.name})`, async ({ page }) => {
  ...
  await expect(page.getByText(/you'?re all set|complete setup/i).first()).toBeVisible();
  await page.getByRole("button", { name: /complete setup/i }).click();

  // Lands on the overview
  await page.waitForURL(/\/overview/, { timeout: 30_000 });
  await page.waitForLoadState("networkidle");
  await page.screenshot({ path: `test-results/s${KEY}-overview.png`, fullPage: true });
  ...
  await page.waitForLoadState("networkidle");
  await expect(page.getByText(new RegExp(incomeWhole)).first()).toBeVisible({ timeout: 15_000 });
  await page.waitForTimeout(1500);
  await page.screenshot({ path: `test-results/s${KEY}-incomes.png`, fullPage: true });

  // Family — the Self member shows the name we entered
  await page.goto("/user?tab=family");
  await page.waitForLoadState("networkidle");
  await expect(page.getByText(new RegExp(S.name, "i")).first()).toBeVisible({ timeout: 15_000 });
  await page.waitForTimeout(1500);
  await page.screenshot({ path: `test-results/s${KEY}-family.png`, fullPage: true });

  // CPF
  await page.goto("/user?tab=cpf");
  await page.waitForLoadState("networkidle");
  await page.waitForTimeout(1500);
  await page.screenshot({ path: `test-results/s${KEY}-cpf.png`, fullPage: true });

  // Holdings — the bank + amount we added
  await page.goto("/user?tab=holdings");
  await page.waitForLoadState("networkidle");
  await expect(page.getByText(new RegExp(S.bank, "i")).first()).toBeVisible({ timeout: 15_000 });
  await page.waitForTimeout(1500);
  await page.screenshot({ path: `test-results/s${KEY}-holdings.png`, fullPage: true });

  // Expenses
  await page.goto("/user?tab=expenses");
  await page.waitForLoadState("networkidle");
  await page.waitForTimeout(1500);
  await page.screenshot({ path: `test-results/s${KEY}-expenses.png`, fullPage: true });
});
