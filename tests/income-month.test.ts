...
import { buildCashflowModel, type CashflowIncomeInput } from "@/lib/cashflow-sankey";
...
describe("grossForMonth", () => {
  ...
  it("is 0 outside the income window", () => {
    expect(grossForMonth(income({ startDate: "2026-06-01" }), "2026-05")).toBe(0);
  });
  ...
  it("applies a permanent raise from its month onward", () => {
    const inc = income({ accountForFutureChange: true, futureMilestones: raise });
  ...
  it("applies a temporary dip only within its window", () => {
    const inc = income({ accountForFutureChange: true, futureMilestones: tempDip });
    expect(grossForMonth(inc, "2026-05")).toBe(5000); // before
  ...
  it("ignores milestones when accountForFutureChange is off", () => {
    const inc = income({ accountForFutureChange: false, futureMilestones: raise });
...
describe("Sankey ⇄ Timeline Studio agreement", () => {
  ...
  function sankeyIncome(overrides: Partial<CashflowIncomeInput> = {}): CashflowIncomeInput {
    return {
      id: "inc1",
      name: "Salary",
      amount: "5000",
