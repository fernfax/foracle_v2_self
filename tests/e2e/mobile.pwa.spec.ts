...
test.fixme("G2 — viewport opts into viewport-fit=cover (iOS safe areas resolve)", async ({
  page
}) => {
  await page.goto("/")
  const content =
...
test.fixme("G4 — iOS standalone enabled (apple-mobile-web-app-capable)", async ({
  page
}) => {
  await page.goto("/")
  const cap = page
...
test.fixme("G6 — a service worker registers and controls the page", async ({
  page
}) => {
  await page.goto("/")
...
test.fixme("G7 — bottom-nav tap targets are >= 44px (needs e2e auth wiring)", async ({
  page
}) => {
...
test.fixme("G8 — offline cold-load serves a cached app shell (needs SW)", async ({
  page,
  context
}) => {
  await page.goto("/")
...
test.fixme("G9 — Budget logs an expense offline and syncs on reconnect (needs SW + queue)", async ({
  page,
  context
}) => {
...
test.fixme("G10 — inputs use >=16px font so iOS Safari does not zoom on focus", async ({
  page
}) => {
  ...
  expect(
    fontPx,
    `input font-size ${fontPx}px < 16 triggers iOS zoom`
