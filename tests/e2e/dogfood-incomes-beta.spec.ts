...
const ARTIFACTS_DIR = path.join(process.cwd(), "test-results", "dogfood-incomes-beta");
...
test.describe("Dogfood: incomes beta view", () => {
  ...
  test("walk the page, capture state and any errors", async ({ page }) => {
    ...
    const landedOnTarget = /\/user/.test(finalUrl) && finalUrl.includes("view=beta");
    ...
    if (landedOnTarget) {
      ...
      const hasBetaBadge =
        (await page.getByText(/beta/i).count()) > 0;
      ...
      const addIncomeVisible = (await addIncomeButton.count()) > 0
        ? await addIncomeButton.first().isVisible()
        : false;

      // Try opening the creator drawer if present
      if (addIncomeVisible) {
        await addIncomeButton.first().click().catch(() => {});
