...
function computeCpfContributions(cpfApplicableAmount, rates) {
  if (cpfApplicableAmount <= CPF_LOW_WAGE_NO_CPF) return { employee: 0, employer: 0 };
...
function calculateCPF(grossAmount, age) {
  ...
  const { employee: employeeRaw, employer: employerRaw } = computeCpfContributions(
    cpfApplicableAmount,
    rates
  );
  ...
  const netTakeHome = Math.round((grossAmount - employeeCpfContribution) * 100) / 100;
...
async function main() {
  ...
  selfCheck();
  console.log(`CPF 2026 backfill — ${APPLY ? "APPLY (writing)" : "DRY RUN (no writes)"}`);
  ...
  try {
    ...
    console.log(`  backfillable (linked member + DOB): ${rows.length - orphans.length}`);
    console.log(`    would change:    ${changes.length}  (age 56–65 rate change: ${rateChanges.length} | rounding-only: ${roundingOnly.length})`);
    console.log(`    already correct: ${unchanged}`);
    console.log(`  skipped — no member / no DOB: ${orphans.length}  (left for member+DOB policy migration, audit PR 5)`);
